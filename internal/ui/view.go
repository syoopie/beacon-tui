...
var (
	...
	warnTextStyle   = lipgloss.NewStyle().Foreground(warnColor)
...
func (serverDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	glyph := lipgloss.NewStyle().Foreground(statusColor(si.status)).Render(statusGlyph(si.status))
	left := marker + glyph + " " + string(si.spec.ID)

	right := si.status.String()
	if si.warn != "" {
		right = "⚠ " + right
	}
	...
	rendered := left + strings.Repeat(" ", gap) + lipgloss.NewStyle().Foreground(statusColor(si.status)).Render(right)
	if selected {
...
func (m *model) commandBar() helpSet {
	...
	case m.pat != nil:
		return helpSet{short: []key.Binding{hint("y", "apply fix"), hint("n", "cancel")}}
...
func (m *model) bodyView() string {
	...
	return lipgloss.NewStyle().Width(m.bodyW).MaxWidth(m.bodyW).
...
func (m *model) paneView() string {
	right := lipgloss.JoinVertical(lipgloss.Left,
		m.logHeaderView(),
		mutedStyle.Render(strings.Repeat("─", max(m.vp.Width, 1))),
		m.vp.View(),
	)
	rows := max(m.bodyH, 1)
...
func (m *model) logHeaderView() string {
	...
	if !spec.Exec.Launchable() {
		parts = append(parts, warnTextStyle.Render("script needs fixing — press p"))
	}
	if r.Warning != "" {
		parts = append(parts, warnTextStyle.Render("⚠ "+r.Warning))
	}
...
func (m *model) patchDialogView() string {
	...
	inner := lipgloss.JoinVertical(lipgloss.Left,
		sectionStyle.Render("Fix the start script for "+string(p.id)+"?"),
		"",
		mutedStyle.Render("beacon can only stop a server whose script ends by exec-ing java."),
		mutedStyle.Render("The original is kept as "+string(p.id)+"'s run.sh.bak."),
		"",
		p.patch.Diff(),
	)
